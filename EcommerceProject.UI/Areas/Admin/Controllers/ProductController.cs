...
namespace EcommerceProject.UI.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> AssignImage(int productID)
        {
            var product = await _productManager.FindAsync(productID);

            var resultProduct = _productManager.Where(x => x.ProductCode == product.ProductCode && x.ProductColorID == product.ProductColorID).FirstOrDefault();

            if (resultProduct != null)
            {
                var assignmentImageDetails = await _imageDetailManager.AssignmentImageDetailsByProductID(resultProduct.ID, productID);

                if (assignmentImageDetails)
                {
                    _toast.AddSuccessToastMessage($"{resultProduct.ProductName} isimli ürün resimleri {product.ProductName} isimli ürüne aktarılmıştır.", new ToastrOptions { Title = "İşlem Başarılı" });
                }
                else
                {
                    _toast.AddErrorToastMessage("Resim ataması yapılabilecek bir hedef ürün bulunamadı ya da seçili ürüne atanmış resimler olabilir.", new ToastrOptions { Title = "İşlem Başarısız" });
                }
            }
            else
            {
                _toast.AddErrorToastMessage("Resim ataması yapılabilecek bir hedef ürün bulunamadı.", new ToastrOptions { Title = "İşlem Başarısız" });
            }
            return RedirectToAction("Index", "Product", new { Area = "Admin" });
        }
